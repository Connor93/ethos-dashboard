...
export class GfxLoader {
  ...
  /**
   * Returns the sorted list of resource IDs present in an EGF after load.
   * Used by the pub-editor graphic picker to enumerate valid graphics
   * without trial-and-error loadResource calls.
   */
  async listResources(fileID: number): Promise<number[]> {
    if (!this.egfs.has(fileID)) {
      await this.loadEGF(fileID);
    }
    const info = this.egfs.get(fileID);
    if (!info) return [];
    return Array.from(info.keys()).sort((a, b) => a - b);
  }
  ...
  private async toImageBitmap(
    ...
    // LRU eviction — don't close() the bitmap because the atlas may still
    // hold a reference to it for character frame composition.  Let the GC
    // reclaim it once all references are dropped.
